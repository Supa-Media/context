...
| **Calendar** | Drives Calendar.app over JXA, which needs **two** permissions, not one: Automation to send it Apple Events at all, and — separately — the same Calendars category `NSCalendarsFullAccessUsageDescription` governs to read the data back, at the full-access tier rather than the write-only one macOS 14+ hands an app that only declares the legacy key. Measured on the owner's Mac: Automation was granted and Calendars was at write-only. Whether that write-only tier actually makes this collector refuse is, as of this pull request, **unknown, not confirmed** — an earlier record inferred a silent refusal from the permission value alone, and a later measurement against the running app found no refusal at all on the same machine; see `docs/decisions/desktop-updates.md`, "The one-way door", for the retraction. The collector counts calendars and refusals either way and `collectSignals` reports a *total* refusal as `degraded`, with the fix in System Settings named in the panel; the two counts themselves reach `DetectionUpdate` as diagnosis, never the panel, so a partial or total refusal is finally observable from outside the app if one ever happens. Also slow. Attendees come back empty rather than invented. The right implementation is EventKit through a native helper, which also gets change notifications instead of a five-second poll and can ask `authorizationStatus(for:)` directly instead of inferring it from an empty result. |
