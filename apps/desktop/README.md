...
| **Calendar** | Drives Calendar.app over JXA, which needs **two** permissions, not one: Automation to send it Apple Events at all, and — separately — the same Calendars category `NSCalendarsFullAccessUsageDescription` governs to read the data back, at the full-access tier rather than the write-only one macOS 14+ hands an app that only declares the legacy key. Measured on the owner's Mac: Automation was granted and Calendars was at write-only — and, at the time, silently: `calendarScript`'s per-calendar refusal read as an empty diary, not as "not allowed to look." That collector now counts refusals and `collectSignals` reports it as `degraded`, with the fix in System Settings named in the panel, per `docs/decisions/desktop-updates.md`. Also slow. Attendees come back empty rather than invented. The right implementation is EventKit through a native helper, which also gets change notifications instead of a five-second poll and can ask `authorizationStatus(for:)` directly instead of inferring it from an empty result. |
