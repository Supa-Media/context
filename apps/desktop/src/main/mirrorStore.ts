...
export class MirrorStore {
   ...
   * `MirrorSaveInput.files` documents its own first entry as the document —
   * "the index a navigation falls back to" — and this used to trust that by
   * writing `index ??= key` inside the loop below: whichever response was
   * stored first became the index, with no check that it was a document at
   * all. When `shouldMirror` was refusing every response carrying
   * `Cache-Control: private` (the console's own document is served with
   * exactly that), the document never survived filtering and the first thing
   * that *did* was the JS bundle — so the manifest's index answered
   * `application/javascript`, `mirrorIsUsable` said yes, and the offline
   * window rendered raw minified JavaScript in a `<pre>`. So the document is
   * checked by name before anything touches disk: if it is missing, or is not
   * `text/html`, the whole snapshot is discarded and the mirror already on
   * disk — the last one that *did* have a real document — is left standing.
   */
  async save(input: MirrorSaveInput): Promise<MirrorManifest | null> {
    const [document] = input.files;
