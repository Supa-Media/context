...
import {
  ...
  decideGrantedScopes,
  noGrantableWorkspace,
  noScopesGranted,
  resolveConsentWorkspace,
} from "./lib/authorizations/requests";
import {
  MACHINE_APPROVAL_LIMIT,
  MACHINE_APPROVAL_WINDOW_MS,
  arm,
  machineApprovalRefused,
} from "./lib/authorizations/approval";
import {
  authorizationRequestReturns,
  readAuthorizationRequest,
} from "./lib/authorizations/consentScreen";

/** What the consent screen needs, and nothing else. */
export const getAuthorizationRequest = query({
  args: { requestId: v.string() },
  returns: authorizationRequestReturns,
  handler: async (ctx, args) => await readAuthorizationRequest(ctx, args),
});
