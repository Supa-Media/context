...
describe("a capture reaches an open console", () => {
  /*
    The email worker writes the note into the bucket itself, with the
    credential its ticket bought, and this route is the only thing the control
    plane hears afterwards. So this is where the owner's tree is told to walk:
    without it, mail sent to a personal context sat unseen until the next
    periodic pass. The owner and nobody else — the path is not sent here (and
  ...
  */
  ...
  test("a duplicate, an invented ticket and a spent one move nothing", async () => {
    const { t } = await ready();
