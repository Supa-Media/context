...
export function canDrop(
  ...
  if (target.kind === "context") {
    ...
    return {
      ok: false,
      reason: `${contextSegment(target.slug)} is separate, with its own bucket. Moving notes between workspaces is not supported yet — open it and create the note there.`,
    };
  }
