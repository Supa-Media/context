 ...
 * (spec A2). The same groups the List view draws, laid out sideways.
 *
 ...
 * is its status value: press it and pick, the same menu everywhere else on
 * the page. The columns scroll sideways when they outgrow the page, on a
 * phone as on a desktop, and a long column scrolls with the page.
 */
...
export function FolderBoard({
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.columns} testID="folder-board">
      {groups.map((group) => (
        <View key={group.value.toLowerCase()} style={[styles.column, compact && styles.columnTouch]} testID="folder-board-column">
          ...
          <View style={styles.cards}>
            {group.items.map((item) => (
              <Card key={item.path} item={item} now={now} actions={actions} compact={compact} />
            ))}
          </View>
        </View>
      ))}
...
function Card({ item, now, actions, compact }: { item: FolderItem; now: number; actions: ItemActions; compact: boolean }) {
  const styles = useThemedStyles(makeStyles);
      ...
      <View style={styles.foot}>
        ...
        {edit === null ? null : (
          <PropertyValue
            ...
            variant="meta"
            // The column already says it; on a pointer the word is the handle, shown with the card.
...
const makeStyles = (colors: Colors) =>
  StyleSheet.create({
    ...
    cards: { gap: space.x2, marginTop: space.x1 },
    card: {
