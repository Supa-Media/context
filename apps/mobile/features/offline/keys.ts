 ...
 * the first mount after the upgrade.
 *
 * This particular bump is free, and the reason is a fact about the history
 * rather than a property of the design: `v1` was introduced by the commit that
 * is currently the tip of `main`, so no build carries it and no device holds a
 * `v1` key. **The next one will not be free.** This app pins one
 * `runtimeVersion` and delivers everything over the air (CLAUDE.md, "One
 * runtime version, pinned"), so an update lands on installs that have been
 * queueing writes for months. Anybody bumping this again should either land it
 * before a release or decide, deliberately, that a queue may be discarded —
 * rather than find out afterwards from somebody whose unsent edits went. The
 * shape of the alternative, if that decision ever goes the other way: only the
 * two *scoped* kinds changed layout here, and an old-shape key is detectable
 * by its segment count, so a migration could retire those and leave the typing
 * alone at the cost of a second "this version cannot read it" rule.
 */
