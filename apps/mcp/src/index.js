 ...
 *
 * What stays here, and why it has not moved yet: the tool handlers, the
 * collaboration/presence authorization, recordChange, the orient survey and
 * the MCP transport are still to be split out by subject. The dispatch switch
 * (`callTool`), `callToolForSession`, TOOL_NAME_ALIASES and UNLISTED_TOOLS are
 * read off whichever ONE module declares them by toolArguments.test.mjs
 * (test/gatewaySource.mjs); IMAGE_MIME_TYPES and MAX_INLINE_IMAGE_BYTES are
 * scraped from this file by the email worker's and control plane's tests.
 * The only module-level mutable state left here is `advertisedSchemas`,
 * written by `advertisedSchemaFor` beside it.
 */
...
import { accessSentence, currentReach } from "./orient/access.js";
import {
  actorFor,
  contextNameFor,
  contextsFor,
